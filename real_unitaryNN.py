...
cost=T.sqr(T.sqr(T.sum(T.transpose(W[3:6])*xreal))+T.sqr(T.sum(T.transpose(W[6:9])*xreal))+T.sqr(T.sum(T.transpose(W[0:3])*xreal))-1)+l*T.sqr(T.sqr(T.sum(T.transpose(W[3:6])*xreal))+T.sqr(T.sum(T.transpose(W[6:9])*xreal))-1)
normip=[]
loss=[]
...
normtest=[]
for j in range(100):       
#        inputvector(10*np.random.rand(1),10*np.random.rand(1))
        inputvector2(10*np.random.rand(1),0.1,0.2,0.3)
        X = np.zeros((3,1))
        testip.append(xreal)
